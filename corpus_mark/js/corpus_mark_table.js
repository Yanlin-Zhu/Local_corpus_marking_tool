$(function(){
		...
		function creatTable(tableHead,bodyData,saveData){
			...
			for(var i = 0 ;i<serialNumber.length;i++){
		        ...
		        $("tbody tr").last().append(tdHtml);
	 			...
	 			if($($('.showLabelbox')[i]).children().length!=0){
	 				$($(".marked")[i]).html('已标注')
	 			}
			}
		}
		...
		function init(){
	 	...
	 	reader.onload = function(progressEvent){
	 	    ...
	 	    if(localStorage.getItem(fileName)!=undefined){
	 	    	var aadd = JSON.parse(localStorage.getItem(fileName));
	 	    }
	 	    sign = {}
	 	    if(aadd!=null){
	 	    	sign[fileName]=aadd;
	        			 	  	...
	        			 	  	if(window.getSelection()!=""){
	        		  			  	...
	        		  			  	var linkLabel = '<div class="labels">'+
	        		  			                    ...
	        		  			                    '<button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">'+
	        		  			                      ...
	        		  			                      '<span class="caret"></span>'+
	        		  			                    '</button>'+
	        		  			                    '<ul class="second_level dropdown-menu" aria-labelledby="dropdownMenu1">'+
	        		  			                    '</ul>'+
	        		  			                    '</div>'+
	        		  								'</div>'
	        		  				...
	        		  				//一级标注菜单
	        		  				  ...
	        		  				  for (var index = 0;index<data_json[Object.keys(data_json)[0]].length;index++){
	        		  				  	$(".second_level").append('<li>'+data_json[Object.keys(data_json)[0]][index]+'</li>');
	        		  				  }
	...
	$(document).on('click','.removeBigl',function(){
		...
		console.log(sign[fileName][keyNumber][num])
		...
		if($(this).parent().parent().children().length==1){
			$(this).parent().parent().parent().parent().prev().children().eq(1).html('未标注')
		}
		$(this).parent().remove()
	})
	$(document).on("click",'.cance',function(){
		...
		$(".pop-up-boxContent .contentCenter").empty()
		$(".pop-up-boxContent .contentCenter2").empty()
	})
	$(document).on("click",'.sav',function(){
	  		...
	  		if(sign[fileName][keyNumber][sentenceNum]!=undefined){
		...
		// sign[fileName][keyNumber][sentenceNum].dialog= lines;
		...
		rightEle.next().find('div.showLabel').last().append(aa)
	...
	if ('download' in document.createElement('a')) {
	    ...
	    $("#excl").on('click', function () {
	        funDownload(JSON.stringify(sign), fileName+'_mark.txt');    
	    });
	}else{
